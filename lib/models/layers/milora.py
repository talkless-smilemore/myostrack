...
class MiLoRALinear(nn.Module):
    """MiLoRA wrapper for nn.Linear.

    MiLoRA decomposes W into principal and minor singular components:
        W = Wp + Wm, Wm = Bm @ Am.

    The principal component Wp is frozen and the minor low-rank factors Am/Bm
    are trainable. With alpha=1.0, the wrapped layer exactly matches the
    original Linear output at initialization.
    """

    def __init__(
        self,
        weight_principal: torch.Tensor,
        ...
        super().__init__()
        self.in_features = int(weight_principal.size(1))
        self.out_features = int(weight_principal.size(0))
        self.rank = int(lora_A.size(0))
        self.alpha = float(alpha)

        self.weight_principal = nn.Parameter(weight_principal.detach().clone(), requires_grad=False)
    ...
    def from_linear(
        ...
        if rank <= 0 or rank > max_rank:
            raise ValueError(f"MiLoRA rank must be in [1, {max_rank}], got {rank}.")
        ...
        principal = weight_f - float(alpha) * (lora_B @ lora_A)
...
def inject_milora_into_backbone(
    ...
    """Replace selected backbone Linear layers with MiLoRA wrappers."""
    if not enable:
        return 0, 0
    ...
    def recurse(parent: nn.Module) -> None:
        ...
        for name, child in list(parent.named_children()):
            if isinstance(child, nn.Linear) and name in names and not isinstance(child, MiLoRALinear):
                setattr(parent, name, MiLoRALinear.from_linear(child, rank, alpha))
                replaced += 1
            else:
                recurse(child)
