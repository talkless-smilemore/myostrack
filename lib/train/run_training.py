...
def init_seeds(seed):
    """固定 Python / NumPy / PyTorch RNG。不再在此处改 cudnn：否则会把 benchmark 关掉，训练极慢。"""
    random.seed(seed)
    np.random.seed(seed)
...
def run_training(script_name, config_name, cudnn_benchmark=True, local_rank=-1, save_dir=None, base_seed=None,
    ...
    # 在 seed 之后设置 cudnn：init_seeds 不再关闭 benchmark（原逻辑会严重拖慢卷积）
...
def main():
    ...
    # Ampere 及以上：TF32 加速 matmul（对 ViT 类 backbone 通常有收益，精度影响很小）
    if torch.cuda.is_available():
